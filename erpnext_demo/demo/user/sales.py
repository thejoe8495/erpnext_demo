...
def work(domain="Manufacturing"):
	...
	if random.random() < 0.3:
		for i in range(random.randint(1, 3)):
			...
			if quotation and quotation.status == "Open":
				quotation.declare_enquiry_lost([{"lost_reason": "Did not ask"}])
...
def make_quotation(domain):
	...
	if opportunity:
		...
		party_account_currency = frappe.get_value("Customer", get_random("Customer"), 'default_currency')
...
def make_sales_order():
	q = get_random("Quotation", {"status": "Open"})
	if q:
		...
		for i in so.payment_schedule:
			i.due_date = frappe.utils.add_days(frappe.flags.current_date, 30)
		so.insert()
		so.transaction_date = frappe.flags.current_date
		so.save()
		frappe.db.commit()
