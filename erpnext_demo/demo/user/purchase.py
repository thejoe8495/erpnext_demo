...
def work():
	...
	if random.random() < 0.6:
		for mr in frappe.get_all(
			"Material Request",
			filters={"material_request_type": "Purchase", "status": "Pending"},
			...
			if not frappe.get_all("Request for Quotation", filters={"material_request": mr.name}, limit=1):
				...
				rfq.message_for_supplier = "Please help me"
				add_suppliers(rfq)
				rfq.save()
				rfq.submit()
	...
	if random.random() < 0.5:
		...
		for row in query_report.run(report)["result"][:random.randint(1, 8)]:
	...
	if random.random() < 0.5:
		...
		report_res = query_report.run(report, {"from_date": frappe.utils.add_days(frappe.flags.current_date, -100), "to_date": frappe.flags.current_date })["result"]
		for idx in range(len(report_res)):
			if not isinstance(report_res[idx], list):
				po = frappe.get_doc(make_purchase_order(report_res[idx]["material_request"]))
				...
				po.set_warehouse = "Stores - WPL"
				for po_items in po.items:
					po_items.schedule_date = frappe.utils.add_days(frappe.flags.current_date, 7)
				po.set_missing_values(po)
				po.insert()
				po.submit()
				frappe.db.commit()
