...
def work():
	...
	for bom in frappe.get_all("BOM", fields=["item"], filters={"with_operations": 1}):
		pro_order = make_wo_order_test_record(
			...
			posting_date = frappe.flags.current_date,
			planned_start_date = str(frappe.flags.current_date) + " " + str(random.randint(7,18))  + ":" + str(random.randint(0,59))  + ":00"
		)
...
def submit_job_cards():
	...
	employee = "HR-EMP-0000" + str(random.randint(1, 9))
